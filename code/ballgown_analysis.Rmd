...
## Identify differentially expressed transcripts between Sex.
...
merged_DEG_sex_filtered_genes_only <- merge(x = DEG_sex_filtered_stat_results, y = genes_BED, by.x = "id", by.y = "V4")
...
DEG_sex_filtered_female <- filter(merged_DEG_sex_filtered, fc < 1)
head(DEG_sex_filtered_female)

# Filter for male up-regulated genes (i.e. fold-change < 1)
DEG_sex_filtered_male <- filter(merged_DEG_sex_filtered, fc > 1)
...
# Creates a BED file of all DEG and inserts necessary columns to create properly formatted BED file. 
write.table(
  (
    merged_DEG_sex_filtered %>% 
    select(chr, start, end, t_name, strand) %>% 
...
# Creates a BED file of only female DEGs and inserts necessary columns to create properly formatted BED file. 
write.table(
  (
    DET_sex_filtered_female %>% 
    select(chr, start, end, t_name, strand) %>% 
...
# Creates a BED file of only male DEGs and inserts necessary columns to create properly formatted BED file. 
write.table(
  (
    DET_sex_filtered_male %>% 
    select(chr, start, end, t_name, strand) %>% 
...
# Count number of DEG.
count_DEG_sex_filtered_stat_results <-  nrow(merged_DEG_sex_filtered)
