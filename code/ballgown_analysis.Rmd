...
# Vector of comparisons to make
comparisons <- c("t2.v.t4", "t1.v.t3", "t2_t4.v.t1_t3", "t1_t2.v.t3_t4")
...
for (index in 1:length(treatment_fpkm_list))  {
  ...
  # Checks is dataframe exists
  if (exists("treatment_fpkm_CoV_df")) {
    
    # Add new column with coefficients of variations for each subsequnt TreatmentN
    treatment_fpkm_CoV_df$column <- rowcvs(treatment_fpkm_list[[index]], unbiased = TRUE)
  }
  
  else {
    # Creates initial dataframe with coefficient of variations for FPKM in Treatment 1
