...
rm(treatment_fpkm_CoV_df, control.vs.exposed_fpkm_CoV_df, female.vs.male_fpkm_CoV_df)
...
for (comparison in comparisons) {
  ...
  for (index in 1:length(treatment_fpkm_list))  {
    ...
    if (comparison == "f_v_m" & (index == "2" || index == "4")) {
      ...
      if (!exists("females_matrix")) {
        females_matrix <- treatment_fpkm_list[[index]]
        
        # If matrix does exist, append column to matrix
      } else
          females_matrix <- cbind(treatment_fpkm_list[[index]])
      ...
      if (exists("female.vs.male_fpkm_CoV_df")) {
        ...
        # Add new column with coefficients of variation for each subsequent TreatmentN
        female.vs.male_fpkm_CoV_df$column <- rowcvs(females_matrix, unbiased = TRUE)
      } else
          # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
          female.vs.male_fpkm_CoV_df <- as.data.frame(rowcvs(females_matrix, unbiased = TRUE))
        ...
        # If matrix doesn't exist, create it
        if (!exists("males_matrix")) {
          males_matrix <- treatment_fpkm_list[[index]]
          
          # If matrix does exist, append column to matrix
        } else
          males_matrix <- cbind(treatment_fpkm_list[[index]])
      
        # Checks if data frame exists
        if (exists("female.vs.male_fpkm_CoV_df")) {
          
          # Add new column with coefficients of variation for each subsequent TreatmentN
          female.vs.male_fpkm_CoV_df$column <- rowcvs(males_matrix, unbiased = TRUE)
        } else
            # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
            female.vs.male_fpkm_CoV_df <- as.data.frame(rowcvs(males_matrix, unbiased = TRUE))
        ...
        # If matrix doesn't exist, create it
        if (!exists("controls_matrix")) {
          controls_matrix <- treatment_fpkm_list[[index]]
          
          # If matrix does exist, append column to matrix
        } else
            controls_matrix <- cbind(treatment_fpkm_list[[index]])
      
        # Checks is data frame exists
        if (exists("control.vs.exposed_fpkm_CoV_df")) {
      
          # Add new column with coefficients of variation for each subsequent TreatmentN
          control.vs.exposed_fpkm_CoV_df$column <- rowcvs(controls_matrix, unbiased = TRUE)
        } else
            # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
            control.vs.exposed_fpkm_CoV_df <- as.data.frame(rowcvs(controls_matrix, unbiased = TRUE))
      
    } # Treatments 3&4 (exposed)
      else if (comparison == "c_v_e" & (index == "3" || index == "4")) {
        
        # If matrix doesn't exist, create it
        if (!exists("exposed_matrix")) {
          exposed_matrix <- treatment_fpkm_list[[index]]
          
          # If matrix does exist, append column to matrix
        } else
            exposed_matrix <- cbind(treatment_fpkm_list[[index]])
      
        # Checks if data frame exists
        if (exists("control.vs.exposed_fpkm_CoV_df")) {
          
          # Add new column with coefficients of variation for each subsequent TreatmentN
          control.vs.exposed_fpkm_CoV_df$column <- rowcvs(exposed_matrix, unbiased = TRUE)
        } else
            # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
            control.vs.exposed_fpkm_CoV_df <- as.data.frame(rowcvs(exposed_matrix, unbiased = TRUE))
    }
  }
}

# Rename columns
names(female.vs.male_fpkm_CoV_df)[1] <- paste("males", "_", "FPKM_Coeff_of_Var", sep = "")
names(female.vs.male_fpkm_CoV_df)[2] <- paste("females", "_", "FPKM_Coeff_of_Var", sep = "")
names(control.vs.exposed_fpkm_CoV_df)[1] <- paste("controls", "_", "FPKM_Coeff_of_Var", sep = "")
names(control.vs.exposed_fpkm_CoV_df)[2] <- paste("exposed", "_", "FPKM_Coeff_of_Var", sep = "")

# Create transcript ID column
control.vs.exposed_fpkm_CoV_df <- control.vs.exposed_fpkm_CoV_df %>% 
  rownames_to_column(var = "t_id")

female.vs.male_fpkm_CoV_df <- female.vs.male_fpkm_CoV_df %>% 
  rownames_to_column(var = "t_id")
...
# Check data frame
head(control.vs.exposed_fpkm_CoV_df)
head(female.vs.male_fpkm_CoV_df)
head(treatment_fpkm_CoV_df)

# Write data frames to CSVs
write.csv(control.vs.exposed_fpkm_CoV_df,
          file = "../analyses/control.vs.exposed_fpkm_CoV.csv",
          row.names = FALSE,
          quote = FALSE)

write.csv(female.vs.male_fpkm_CoV_df,
          file = "../analyses/female.vs.male_fpkm_CoV.csv",
          row.names = FALSE,
          quote = FALSE)

write.csv(treatment_fpkm_CoV_df,
          file = "../analyses/treatment_fpkm_CoV.csv",
          row.names = FALSE,
          quote = FALSE)
```
...
merged_whole_gx_table_t_rownames <- merge(whole_gx_table_t_rownames_sex, phenotype_table, by = "OldSample.ID") %>% relocate(OldSample.ID, Treatment, TreatmentN, Sex, .after = library)

# Rename FPKM summed column
rename(merged_whole_gx_table_t_rownames, FPKM_sum = ".")
