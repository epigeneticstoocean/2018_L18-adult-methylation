...
for (comparison in comparisons) {
  ...
  for (index in 1:length(treatment_fpkm_list))  {
    ...
    if (comparison == "f_v_m" && index == "2" || index == "4") {
      if (!exists(females_df)) {
        females_df <- as.data.frame(treatment_fpkm_list[[index]])
      }
      else {
        females_df$column <- treatment_fpkm_list[[index]]
      }
      ...
      # Renames column
      rename(female.vs.male_fpkm_CoV_df, treatment_col2 = column)
    ...
    # Treatments 1&3 (males)
      ...
      if (!exists(males_df)) {
        ...
        males_df$column <- treatment_fpkm_list[[index]]
      }
      ...
      # Renames column 2
      rename(female.vs.male_fpkm_CoV_df, treatment_col1 = column)
      
    }
    # Treatments 1&2 (control)
    else if (comparison == "c_v_e" && index == "1" || index == "2") {
      
      if (!exists(controls_df)) {
        controls_df <- as.data.frame(treatment_fpkm_list[[index]])
      }
      else {
        controls_df$column <- treatment_fpkm_list[[index]]
      }
      ...
      # Add new column with coefficients of variation for each subsequent TreatmentN
      control.vs.exposed_fpkm_CoV_df$column <- rowcvs(controls_df, unbiased = TRUE)
      }
      else {
      # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
      control.vs.exposed_fpkm_CoV_df <- as.data.frame(rowcvs(controls_df, unbiased = TRUE))
      }
    ...
    # Treatments 3&4 (exposed)
      ...
      if (!exists(exposed_df)) {
        exposed_df <- as.data.frame(treatment_fpkm_list[[index]])
      }
      else {
        exposed_df$column <- treatment_fpkm_list[[index]]
      }
      ...
      # Add new column with coefficients of variation for each subsequent TreatmentN
      control.vs.exposed_fpkm_CoV_df$column <- rowcvs(exposed_df, unbiased = TRUE)
      }
      else {
      # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
      control.vs.exposed_fpkm_CoV_df <- as.data.frame(rowcvs(exposed_df, unbiased = TRUE))
      }
    }
    }
  }
