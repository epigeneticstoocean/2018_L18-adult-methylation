...
ballgown_genes_df <- 
  ...
  rownames_to_column(var = "gened_id")
## Extracts gene ids
genes <- whole_gx_table_df$gene_id
  ...
  for (index in 1:length(transcripts_treatment_fpkm_list))  {
    ...
    if (comparison == "f_v_m" & (index == "2" || index == "4")) {
      ...
      if (!exists("transcripts_fetranscripts_males_matrix")) {
        transcripts_fetranscripts_males_matrix <- transcripts_treatment_fpkm_list[[index]]
        
        # If matrix does exist, append column to matrix
      } else
          transcripts_fetranscripts_males_matrix <- cbind(transcripts_treatment_fpkm_list[[index]])
