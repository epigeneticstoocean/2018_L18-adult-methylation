...
# Convert to BED coordinate system, which is 0-based; so need to subtract 1 from each coordinate
merged_DET_sex_filtered <- merged_DET_sex_filtered %>% mutate(start = start -1, end = end -1)
head(merged_DET_sex_filtered)
...
# Convert to BED coordinate system, which is 0-based; so need to subtract 1 from each coordinate
merged_DEG_sex_filtered <- merged_DEG_sex_filtered %>% mutate(start = start -1, end = end -1)
head(merged_DEG_sex_filtered)
...
count_DEG_sex_filtered_stat_results <-  nrow(DEG_sex_filtered_stat_results) - 1
