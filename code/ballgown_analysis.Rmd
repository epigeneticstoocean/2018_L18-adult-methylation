    ...
    if (comparison == "f_v_m" && index == "2" || index == "4") {
      ...
      rename(female.vs.male_fpkm_CoV_df, treatment_col1 = column)
        ...
        if (!exists("controls_df")) {
          controls <- treatment_fpkm_list[[index]]
        } else
            controls <-cbind() treatment_fpkm_list[[index]])
      
        # Checks is data frame exists
        if (exists("control.vs.exposed_fpkm_CoV_df")) {
      
          # Add new column with coefficients of variation for each subsequent TreatmentN
          control.vs.exposed_fpkm_CoV_df$column <- rowcvs(controls, unbiased = TRUE)
        } else
            # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
            control.vs.exposed_fpkm_CoV_df <- as.data.frame(rowcvs(controls, unbiased = TRUE))
