...
for (treatment in treatments) {
  ...
  treatment_fpkm_list[[treatment]] <- texpr(bg_transcripts) 
}
  ...
  for (index in 1:length(treatment_fpkm_list))  {
    if (comparison == "all") {
      ...
      if (exists("genes_treatment_fpkm_CoV_df")) {
        ...
        # Add new column with coefficients of variation for each subsequent TreatmentN
        genes_treatment_fpkm_CoV_df$column <- 
      } else
          # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
          genes_treatment_fpkm_CoV_df <- 
    ...
    if (comparison == "f_v_m" & (index == "2" || index == "4")) {
      ...
      if (!exists("transcripts_fetranscripts_males_matrix")) {
        transcripts_fetranscripts_males_matrix <- treatment_fpkm_list[[index]]
        
        # If matrix does exist, append column to matrix
      } else
          transcripts_fetranscripts_males_matrix <- cbind(treatment_fpkm_list[[index]])
      ...
      if (exists("transcripts_female.vs.male_fpkm_CoV_df")) {
        ...
        # Add new column with coefficients of variation for each subsequent TreatmentN
        transcripts_female.vs.male_fpkm_CoV_df$column <- rowcvs(transcripts_fetranscripts_males_matrix, unbiased = TRUE)
      } else
          # Creates initial data frame with coefficients of variation for FPKM in Treatment 1
          transcripts_female.vs.male_fpkm_CoV_df <- as.data.frame(rowcvs(transcripts_fetranscripts_males_matrix, unbiased = TRUE))
        ...
        if (!exists("transcripts_males_matrix")) {
          transcripts_males_matrix <- treatment_fpkm_list[[index]]
          
          # If matrix does exist, append column to matrix
        } else
          transcripts_males_matrix <- cbind(treatment_fpkm_list[[index]])
        ...
        if (!exists("transcripts_controls_matrix")) {
          transcripts_controls_matrix <- treatment_fpkm_list[[index]]
          
          # If matrix does exist, append column to matrix
        } else
            transcripts_controls_matrix <- cbind(treatment_fpkm_list[[index]])
        ...
        if (!exists("transcripts_exposed_matrix")) {
          transcripts_exposed_matrix <- treatment_fpkm_list[[index]]
          
          # If matrix does exist, append column to matrix
        } else
            transcripts_exposed_matrix <- cbind(treatment_fpkm_list[[index]])
