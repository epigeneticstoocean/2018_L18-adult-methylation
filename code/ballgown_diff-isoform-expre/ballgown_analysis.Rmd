...
## Identify differentially expressed isofroms between Treatment, applying the confounder "Sex".
## Returns FoldChange, too (getFC = TRUE)
```{r}
stat_results = stattest(bg, feature='transcript', meas='FPKM', covariate="Treatment", adjustvars = 'Sex', getFC = TRUE)
stat_results
filtered_stat_results <- filter(stat_results, pval <= pvalue & qval <= qvalue)
filtered_stat_results
nrow(filtered_stat_results)
...
## Identify differentially expressed isofroms between Treatments, without confounder.
## Returns FoldChange, too (getFC = TRUE)
```{r}
...
## Identify differentially expressed isofroms between Sex, without confounder.
## Returns FoldChange, too (getFC = TRUE)
```{r}
