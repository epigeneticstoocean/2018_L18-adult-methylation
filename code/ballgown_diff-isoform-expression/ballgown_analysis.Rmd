...
DET_sex_filtered_stat_results <- filter(stat_results, pval <= pvalue & qval <= qvalue)
...
merged_DET_sex_filtered <- merge(x = DET_sex_filtered_stat_results, y = full_table, by.x = "id", by.y = "t_id")
...
# Creates a BED file and inserts necessary rows to create properly formatted BED file. 
write.table(
  (
    merged_DET_sex_filtered %>% 
    select(id, start, end, strand) %>% 
      add_column(
        name = ".", # Inserts a "name" column and assigns value of "." to all rows
