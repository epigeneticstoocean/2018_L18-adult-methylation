...
# Male stats
transcript_counts_males <- transcript_counts %>%
  rowwise() %>% 
  select(ends_with('M')) %>% 
  mutate(
...
# Female stats
transcript_counts_females <- transcript_counts %>%
  rowwise() %>% 
  select(ends_with('F')) %>% 
  mutate(
...
# Controls stats
transcript_counts_controls <- transcript_counts %>%
  rowwise() %>% 
  select(ends_with(controls)) %>% 
  mutate(
...
# Exposed stats
transcript_counts_exposed <- transcript_counts %>%
  rowwise() %>% 
  select(ends_with(exposed)) %>% 
  mutate(
...
transcript_counts_controls_females <- transcript_counts %>%
  rowwise() %>% 
  select(ends_with(controls_females)) %>% 
  mutate(
...
transcript_counts_exposed_females <- transcript_counts %>%
  rowwise() %>% 
  select(ends_with(exposed_females)) %>% 
  mutate(
...
list_transcript_counts_dfs <- mget(grep("^(?!list).*", ls(pattern = "transcript_counts"), value = TRUE, perl = TRUE))

head(transcript_counts)

# Write to CSV
write.csv(transcript_counts, file = "../analyses/transcript-counts_per-gene-per-sample.csv", row.names = FALSE, quote = FALSE)
